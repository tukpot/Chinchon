...
public class Vista2D extends VistaBase {
	...
	public Vista2D(Controlador controlador) {
		...
		JButton botonTop = new JButton("Top");
		botonTop.setFont(new Font("SansSerif", Font.PLAIN, 10));
		botonTop.setMargin(new Insets(2, 5, 2, 5)); // botón compacto
		botonTop.setFocusable(false);
		botonTop.addActionListener(e -> {
			// aquí llamamos a MiniTopJugadores
			try {
				abrirTopJugadores();
			} catch (RemoteException e1) {
				// TODO Auto-generated catch block
		...
		panelAcciones.add(botonTop, BorderLayout.WEST);
	...
	public void abrirTopJugadores() throws RemoteException {
		String topJugadores = this.getJugadoresTopString();
		String ejemploTop = topJugadores;
		MiniTopJugadores ventanaTop = new MiniTopJugadores(ejemploTop);
